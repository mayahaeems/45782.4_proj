...
class Cart(db.Model):
    ...
    status = db.Column(
        db.Enum(CartStatus, name="cart_status_enum"),
        nullable=False,
    ...
    def get_or_create_active(user_id: int) -> "Cart":
        ...
        if cart:
            return cart
        Cart.query.filter_by(user_id=user_id, status=CartStatus.converted).delete()
...
class CartItem(db.Model):
    ...
    unit_amount = db.Column(db.Integer, nullable=False)  # snapshot price at add time
