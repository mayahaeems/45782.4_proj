...
# ── PUBLIC REGISTER (role forced to USER) ─────────────────────────────────────
class UserCreateSchema(BaseSchema):
    ...
    def validate_phone_digits(self, value, **kwargs):
...
class UserUpdateSchema(BaseSchema):
    ...
    @validates("default_phone")
    def validate_phone_digits(self, value, **kwargs):
        if value is not None and not value.replace("+", "").replace("-", "").isdigit():
            raise ValidationError("Phone number must contain only digits, +, or -")
